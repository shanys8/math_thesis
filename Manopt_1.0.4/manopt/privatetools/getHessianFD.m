...
% Change log: 
    ...
    % TODO: to make this approximation of the Hessian radially linear, that is,
	% to have thate HessianFD(x, a*d) = a*HessianFD(x, d) for all points x,
	% tangent vectors d and real scalars a, we need to pay special attention to
	% the case of a < 0. This requires a notion of "sign" for vectors d.
	% If vectors are uniquely represented by a matrix (which is the case for
	% Riemannian submanifolds of the space of matrices), than this function is
	% appropriate:
    % sg = sign(d(find(d(:), 1, 'first')));
	% But it is more difficult to build such a function in general. For now, we
	% ignore this difficulty and let the sign always be +1. This hardly impacts
	% the actual performances, but may be an obstacle for theoretical analysis.
    sg = 1;
