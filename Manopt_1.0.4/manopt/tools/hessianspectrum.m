...
% projected on the tangent space to avoid unwanted contributions of the
% ambient space.
%
% Requires the manifold description in problem.M to have these functions:
% 
%  u_vec = vec(x, u_mat) : returns a column vector representation of the
%                          normal (usually matrix) representation of the
%                          tangent vector u_mat. vec must be an isometry
%                          between the tangent space (with its Riemannian
%                          metric) and a subspace of R^n where
%                          n = length(u_vec), with the 2-norm on R^n.
%                          In other words: it is an orthogonal projector.
%
% u_mat = mat(x, u_vec) : the inverse of vec (its adjoint).
%
% u_mat_clean = tangent(x, u_mat) : subtracts from the tangent vector u_mat
%                                   any component that would make it "not
%                                   really tangent", by projection.
%
...
% Change log:
    ...
    % necessarily, n >= dim.
    % The vectorized operators we build below will have at least n-dim zero
    % eigenvalues: those correspond to 
    n = length(vec(problem.M.zerovec(x)));
    dim = problem.M.dim();
    
    % The store structure is not updated by the getHessian call because the
    % eigs function will not take care of it. This might be worked around I
    % suppose, but for now we simply obtain the store structure built from
    % calling the cost and gradient at x and pass that one for every
    % Hessian call. This will typically be enough, seen as the Hessian is
    % not supposed to store anything new.
    storedb = struct();
    if canGetGradient(problem)
        [unused1, unused2, storedb] = getCostGrad(problem, x, struct());
    end
    
    hess = @(u_mat) tgt(getHessian(problem, x, tgt(u_mat), storedb));
    hess_vec = @(u_vec) vec(hess(mat(u_vec)));
    
    % Whatever the situation is with preconditioners, we can only have a
    % symmetric eigenvalue problem if the vec/mat pair of the manifold is
    % an isometry:
