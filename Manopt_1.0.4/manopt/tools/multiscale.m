...
% Change log: 
	...
	assert(ndims(A) <= 3, ['multiscale is only well defined for matrix ' ...
	                       'arrays of 3 or less dimensions.']);
	[n m N] = size(A);
	assert(numel(scale) == N, ['scale must be a vector whose length equals ' ...
	                           'the third dimension of A, that is, the number' ...
							   ' of 2D matrix slices in the 3D matrix A.']);
