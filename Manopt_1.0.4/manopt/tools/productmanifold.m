...
%                     Modified hash function to make hash strings shorter.
    ...
    assert(nelems >= 1, 'elements must be a structure with at least one field.');
    ...
    % TODO: By concatenating the hashes, we let the hash length increase with
	% the size of the product. This in turn makes it more expensive to edit and
	% lookup elements in the storedb. Should we re-hash the concatenated hash?
    M.hash = @hash;
    function str = hash(x)
        str = '';
        for i = 1 : nelems
