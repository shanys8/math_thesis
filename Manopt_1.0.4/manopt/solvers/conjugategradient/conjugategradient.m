...
%
%   tolgradnorm (1e-6)
%       The algorithm aborts if the norm of the gradient drops below this.
%   maxiter (1000)
%       The algorithm aborts if maxiter iterations have been executed.
%   maxtime (Inf)
%       The algorithm aborts if maxtime seconds elapsed.
%   minstepsize (1e-10)
%       The algorithm aborts if the linesearch returns a displacement
...
%    linesearch to decide whether they want to use the norm of the search
%    direction in their algorithm or not. There are reasons against it, but
%    practical evidence that it may help too, so we allow it. The default
%    linesearch_adaptive used does exploit the norm information. The base
%    linesearch does not. You may select it by setting
